ES Hub 1.0.1
Removed:
PantheonAPI section and Hermes bridge — Pantheon is retired; do not ship PantheonAPI.
Added:
Grey recommended Utility badges for Calm The Leaks, WhatLIB, Hermes, and Aegis when those mods are not installed.
Changed:
Hermes and Aegis register as normal F9 → Utility tools (no Pantheon dock path).
ESS / Utility / Mods remain the hub sections.
...
- Utility tab shares Other Mods search / AuthorÂ·TitleÂ·Group filter chrome (third-party ops tools plug in with `EshSection.UTILITY`).
- `EshOwnsHubNav` â€” hosted screens that already have their own way back can opt out of ESH's injected `â† Hub` button; ESC's `EscOwnsBackNav` is honoured too, so ESC screens (e.g. the colour picker) opt out without depending on ESH.
